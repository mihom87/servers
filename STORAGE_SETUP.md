...
**Текущие селекторы для servers.com:**
...
# Кнопка Accept
accept_button = web_driver.page.locator('div.cky-consent-bar button[aria-label="Accept"]')
```

**Если селекторы не работают для другого сайта:**

1. Откройте сайт в браузере (F12 → DevTools)
2. Найдите cookie popup и кнопку Accept
3. Скопируйте селекторы
4. Обновите в `conftest.py` (строка ~45-50):

```python
# Замените на ваши селекторы
popup_locator = web_driver.page.locator("ваш-селектор-попапа")
